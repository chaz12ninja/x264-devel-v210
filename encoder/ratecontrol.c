...
void x264_ratecontrol_init_reconfigurable( x264_t *h, int b_init )
{
    ...
    if( h->param.rc.i_vbv_max_bitrate > 0 && h->param.rc.i_vbv_buffer_size > 0 )
    {
        ...
        int kilobit_size = h->param.i_avcintra_compat ? 1024 : 1000;
        ...
        if( rc->b_vbv_min_rate )
            rc->bitrate = (float)h->param.rc.i_bitrate * kilobit_size;
...
static int update_vbv( x264_t *h, int bits )
{
    ...
    if( h->sps->vui.hrd.b_cbr_hrd && rct->buffer_fill_final > buffer_size )
    {
        ...
        bits = h->param.i_avcintra_compat ? filler * 8 : X264_MAX( (FILLER_OVERHEAD - h->param.b_annexb), filler ) * 8;
        rct->buffer_fill_final -= (uint64_t)bits * h->sps->vui.i_time_scale;
    }
    else
